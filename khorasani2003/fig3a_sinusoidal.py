"""
Figure 3a: Band structure for sinusoidal refractive index profile.
Computes TE and TM modes at theta=0 and theta=pi/4.
"""
import sys
import os
import numpy as np
...
from dtmm import n_sinusoidal, compute_bandstructure_full
...
for pol, theta, ls, label in cases:
    omega, kappa, cos_kL = compute_bandstructure_full(
        OMEGA_MAX, n_sinusoidal, theta, pol, N=N_LAYERS, n_points=N_POINTS
    )
    mask = ~np.isnan(kappa)
...
ax.set_title("(a) Sinusoidal profile")
...
fig.savefig(os.path.join(outdir_plots, "fig3a_sinusoidal.png"), dpi=150, bbox_inches="tight")
plt.close(fig)
...
np.savetxt(os.path.join(outdir_data, "fig3a_sinusoidal.csv"), arr, delimiter=",",
