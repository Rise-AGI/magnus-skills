"""
Figure 3b: Band structure for triangular refractive index profile.
"""
import sys
import os
import numpy as np
...
from dtmm import n_triangular, compute_bandstructure_full
...
for pol, theta, ls, label in cases:
    omega, kappa, cos_kL = compute_bandstructure_full(
        OMEGA_MAX, n_triangular, theta, pol, N=N_LAYERS, n_points=N_POINTS
    )
    mask = ~np.isnan(kappa)
...
ax.set_title("(b) Triangular profile")
...
fig.savefig(os.path.join(outdir_plots, "fig3b_triangular.png"), dpi=150, bbox_inches="tight")
plt.close(fig)
...
np.savetxt(os.path.join(outdir_data, "fig3b_triangular.csv"), arr, delimiter=",",
