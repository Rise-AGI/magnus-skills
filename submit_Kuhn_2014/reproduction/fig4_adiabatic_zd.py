"""
Figure 4: Adiabatic preparation of the ground state - Zd model.
...
for N in N_values:
    for d in d_values:
        ...
        for T in T_values:
            try:
                overlap, energy_error = adiabatic_evolution(N, d, xF, T, n_steps, 'zd')
                overlaps.append(overlap)
                errors.append(energy_error)
                print(f"  Zd N={N} d={d} T={T:.1f}: overlap={overlap:.6f}, eps={energy_error:.2e}")
            except Exception as e:
                overlaps.append(np.nan)
                errors.append(np.nan)
                print(f"  Zd N={N} d={d} T={T:.1f}: FAILED ({e})")
        results[key] = {'overlaps': overlaps, 'errors': errors}

# Save data
os.makedirs('../data', exist_ok=True)

with open('../data/fig4_adiabatic_zd.csv', 'w') as f:
    header = 'T'
    for N in N_values:
        for d in d_values:
...
print("Data saved to ../data/fig4_adiabatic_zd.csv")
...
ax.set_title(r'$Z_d$ model: Adiabatic preparation overlap')
ax.legend()
...
plt.tight_layout()
plt.savefig('../plots/fig4_adiabatic_zd.png', dpi=150, bbox_inches='tight')
print("Plot saved to ../plots/fig4_adiabatic_zd.png")
